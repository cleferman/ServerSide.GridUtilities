    ...
    public class SortTypeTests
    {
        ...
        public void SortTypeDeSerialization_AllMembers_Success()
        {
            foreach (SortType sortType in Enum.GetValues(typeof(SortType)))
            {
                ...
                var model = JsonSerializer.Deserialize<SortModel>(JsonSerializer.Serialize(sortModel));

                Assert.IsNotNull(model);
                Assert.That(model.Sort == sortType);
            }
        }

        [Test]
        public void SortTypeDeSerialization_Descc_InvalidFilterType()
        {
